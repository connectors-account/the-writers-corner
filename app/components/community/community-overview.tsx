    ...
    try {
      ...
      if (response.ok) {
        ...
        setPosts(prev => prev.map(p => 
          p.id === postId 
            ? { ...p, likeCount: data.likeCount, userHasLiked: data.userHasLiked }
            : p
        ))
      }
    } catch (error) {
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          className={`text-forest hover:text-rust transition-colors ${post.userHasLiked ? 'text-rust' : ''}`}
                        ...
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          className={`text-forest hover:text-rust transition-colors ${expandedComments.has(post.id) ? 'text-rust' : ''}`}
                    ...
                    <AnimatePresence>
                      {expandedComments.has(post.id) && (
                        <motion.div
                          ...
                          className="mt-4 pt-4 border-t-2 border-ink/10"
                        >
                                    ...
                                    <div key={comment.id} className="bg-sepia/30 rounded p-3">
                                      ...
                                      {editingCommentId === comment.id ? (
                                        <Textarea
                                          ...
                                          className="text-sm font-serif border border-ink/20 focus:border-rust min-h-[60px]"
                                          autoFocus
                                        />
                                      ) : (
