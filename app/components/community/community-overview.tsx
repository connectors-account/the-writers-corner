...
export function CommunityOverview() {
  ...
  const handleEditComment = async (commentId: string, postId: string) => {
    if (!editingComment || editingComment.content.trim().length === 0) return
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          className={`transition-colors ${post.isLiked ? 'text-rust' : 'text-forest hover:text-rust'}`}
                        ...
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          className={`text-forest hover:text-rust ${expandedComments[post.id] ? 'text-rust' : ''}`}
                                  ...
                                  (comments[post.id] || []).map((comment) => (
                                    <div key={comment.id} className="bg-sepia/30 rounded p-3">
                                      <div className="flex items-start justify-between">
                                            ...
                                            <div className="space-y-2">
                                              <Textarea
                                                ...
                                                onChange={(e) => setEditingComment({ 
                                                  ...editingComment, 
                                                  content: e.target.value.slice(0, MAX_COMMENT_LENGTH) 
                                                })}
                                                className="font-serif text-sm min-h-[60px] border-ink"
                                              ...
                                              <div className="flex items-center justify-between">
                                                ...
                                                <div className="flex gap-2">
                                                  ...
                                                  <Button 
                                                    ...
                                                    disabled={submittingComment || editingComment.content.trim().length === 0}
                                        ...
                                        {session?.user?.id === comment.user.id && !editingComment && (
                              ...
                              <div className="flex gap-2">
                                <div className="flex-1 relative">
                                  <Textarea
                                    ...
                                    className="font-serif text-sm min-h-[60px] pr-16 border-2 border-ink focus:border-rust"
                                    maxLength={MAX_COMMENT_LENGTH}
                                  />
                                  <span className="absolute bottom-2 right-2 text-xs text-forest/60">
                                    {(newComment[post.id] || '').length}/{MAX_COMMENT_LENGTH}
                                  </span>
                                </div>
                                <Button
