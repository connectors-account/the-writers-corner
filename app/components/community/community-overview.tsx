...
export function CommunityOverview() {
  ...
  const handleLike = async (postId: string, isLiked: boolean) => {
    try {
      ...
      const response = await fetch(`/api/community/posts/${postId}/like`, { method })
      
      if (response.ok) {
        const data = await response.json()
        setPosts(posts.map(post => 
          post.id === postId 
            ? { ...post, isLikedByUser: !isLiked, likeCount: data.likeCount }
            : post
        ))
                    ...
                    {expandedComments.has(post.id) && (
                      <div className="mt-6 pt-6 border-t-2 border-sepia">
                        ...
                        {loadingComments.has(post.id) ? (
                          ...
                          <div className="text-center py-6 text-forest font-serif text-sm">
