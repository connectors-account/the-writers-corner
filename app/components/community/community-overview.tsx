...
export function CommunityOverview() {
  ...
  const fetchComments = async (postId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        setPostComments(prev => new Map(prev).set(postId, data.comments || []))
      }
    } catch (error) {
  ...
  const handleSubmitComment = async (postId: string) => {
    const content = newComment.get(postId)?.trim()
    if (!content) return
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPostComments(prev => {
          ...
          return new Map(prev).set(postId, [...comments, data.comment])
        })
  ...
  const handleEditComment = async (postId: string, commentId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPostComments(prev => {
          const comments = prev.get(postId) || []
          return new Map(prev).set(postId, comments.map(c => 
  ...
  const handleDeleteComment = async (postId: string, commentId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        setPostComments(prev => {
          const comments = prev.get(postId) || []
          return new Map(prev).set(postId, comments.filter(c => c.id !== commentId))
        })
        // Update comment count
        setPosts(posts.map(post => 
          post.id === postId 
  ...
  const getUserDisplayName = (user: CommentUser | { firstName?: string; lastName?: string; name?: string }) => {
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          className={`${post.liked ? 'text-rust' : 'text-forest'} hover:text-rust`}
                        ...
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          className={`${expandedComments.has(post.id) ? 'text-rust' : 'text-forest'} hover:text-rust`}
                              ...
                              <Textarea
                                ...
                                onChange={(e) => {
                                  ...
                                  if (value.length <= MAX_COMMENT_LENGTH) {
                                    setNewComment(prev => new Map(prev).set(post.id, value))
                                  }
                                }}
                                className="font-serif border-2 border-ink focus:border-rust resize-none pr-20"
                                rows={2}
                              />
                              <div className="absolute bottom-2 right-2 flex items-center gap-2">
                                <span className={`text-xs ${(newComment.get(post.id)?.length || 0) >= MAX_COMMENT_LENGTH ? 'text-rust' : 'text-forest'}`}>
                                  {newComment.get(post.id)?.length || 0}/{MAX_COMMENT_LENGTH}
                                </span>
                                <Button
                                  size="sm"
                                  onClick={() => handleSubmitComment(post.id)}
                                  disabled={!newComment.get(post.id)?.trim() || submittingComment.has(post.id)}
                                  className="btn-vintage text-xs h-7"
                                >
                          ...
                          {loadingComments.has(post.id) ? (
                            ...
                            <div className="space-y-3">
                              {(postComments.get(post.id) || []).length === 0 ? (
                                <p className="text-center font-serif text-forest/70 py-2">
                                    ...
                                    {editingComment === comment.id ? (
                                      <div className="space-y-2">
                                        <Textarea
                                          ...
                                          className="font-serif border-2 border-ink focus:border-rust resize-none text-sm"
                                          rows={2}
                                        />
                                        <div className="flex items-center justify-between">
                                          <span className={`text-xs ${editContent.length >= MAX_COMMENT_LENGTH ? 'text-rust' : 'text-forest'}`}>
                                          ...
                                          <div className="flex items-center gap-2">
                                            <Button
                                              variant="ghost"
                                              size="sm"
                                              className="h-7 text-forest hover:text-rust"
                                              onClick={cancelEdit}
                                            >
                                            ...
                                            <Button
                                              size="sm"
                                              className="btn-vintage h-7 text-xs"
