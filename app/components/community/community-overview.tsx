...
export function CommunityOverview() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPosts(prev => prev.map(post => 
          post.id === postId 
            ? { ...post, likeCount: data.likeCount, isLikedByUser: !isCurrentlyLiked }
            : post
        ))
      }
    } catch (error) {
  ...
  const handleSubmitComment = async (postId: string) => {
    ...
    if (!content || commentLoading.has(postId)) return
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <Button 
                          ...
                          onClick={() => handleLikeToggle(post.id, post.isLikedByUser)}
                          ...
                          {post.comments.length > 0 && (
                            <div className="space-y-4 mb-4">
                              {post.comments.map((comment) => (
                                <div key={comment.id} className="bg-parchment/50 p-3 rounded-sm border border-sepia">
