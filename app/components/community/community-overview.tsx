    ...
    try {
      ...
      if (response.ok) {
        ...
        setPosts(prevPosts =>
          prevPosts.map(post =>
            post.id === postId
              ? { ...post, likeCount: data.likeCount, isLikedByCurrentUser: data.liked }
              : post
          )
        )
      }
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          className={`text-forest hover:text-rust ${post.isLikedByCurrentUser ? 'text-rust' : ''}`}
                          onClick={() => handleLikeToggle(post.id)}
                          disabled={likingPosts.has(post.id)}
                        >
                          <Heart className={`w-4 h-4 mr-1 ${post.isLikedByCurrentUser ? 'fill-rust' : ''}`} />
                        ...
                        {loadingComments.has(post.id) ? (
                          ...
                          <div className="space-y-3">
                            {comments[post.id].map((comment) => (
                              <div key={comment.id} className="bg-sepia/30 p-3 rounded-sm border border-forest/20">
                                <div className="flex items-start justify-between mb-2">
                                  <div>
                                    ...
                                    <span className="text-xs text-forest/70 ml-2">
                                  ...
                                  {session?.user?.id === comment.user.id && (
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      className="text-rust hover:text-rust/80 h-6 px-2 text-xs flex items-center gap-1"
                          ...
                          <div className="text-center py-4 text-forest/70 font-serif text-sm">
