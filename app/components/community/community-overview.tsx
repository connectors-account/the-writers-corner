...
import { Users, PenTool, BookOpen, Heart, MessageCircle, Filter, Search, Edit2, Trash2, X } from 'lucide-react'
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPosts(posts.map(post => 
          post.id === postId 
            ? { ...post, likeCount: data.count, isLikedByUser: data.isLiked }
            : post
        ))
      }
    } catch (error) {
                ...
                <Card className="card-vintage border-2 hover:shadow-xl transition-all duration-300">
                  ...
                  <CardContent>
                    ...
                    {expandedComments.has(post.id) && (
                      <div className="mt-4 pt-4 border-t-2 border-sepia/30">
                        {/* Comment Form */}
                        <div className="mb-4">
                          <Textarea
                        ...
                        <div className="space-y-3">
                          {comments[post.id]?.map((comment) => (
                            <div key={comment.id} className="bg-sepia/20 p-3 rounded border border-sepia">
