    ...
    try {
      ...
      if (response.ok) {
        ...
        setPosts(prevPosts =>
          prevPosts.map(post =>
            post.id === postId
              ? { ...post, hasLiked: data.hasLiked, likeCount: data.likeCount }
              : post
          )
        )
      }
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          className={`text-forest hover:text-rust ${post.hasLiked ? 'text-rust' : ''}`}
                          onClick={() => handleLike(post.id)}
                          disabled={likingPosts.has(post.id)}
                        >
                          <Heart className={`w-4 h-4 mr-1 ${post.hasLiked ? 'fill-rust' : ''}`} />
                        ...
                        >
                          ...
                          <div className="flex gap-2 mb-4">
                            <Textarea
                              ...
                              className="font-serif border-2 border-ink focus:border-rust resize-none min-h-[60px]"
                              rows={2}
                            />
                            <Button
                              size="sm"
